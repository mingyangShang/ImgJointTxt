...
class AdvCrossModalSimple(BaseModel):
    ...
    def train(self, sess):
        ...
        emb_train_op = tf.train.AdamOptimizer(
            ...
            beta1=0.5).minimize(self.G_loss, var_list=self.le_vars+self.vf_vars+self.pc_vars)
        ...
        for epoch in range(self.model_params.epoch):
            ...
            for batch_feat, batch_vec, batch_labels, idx in self.data_iter.train_data():
                ...
                #                     self.y_single: np.transpose([batch_labels]),
                #                     self.l: l})
                label_loss_val, triplet_loss_val, emb_loss_val, domain_loss_val, pair_loss_val, g_loss_val, domain_acc_val, label_acc_val, pair_all_acc_val, pair_acc_val, unpair_acc_val = \
                    sess.run([self.label_loss, self.triplet_loss, self.emb_loss, self.domain_class_loss, self.pair_loss, self.G_loss, self.domain_class_acc, self.label_class_acc, self.pair_all_acc, self.pair_acc, self.unpair_acc],
                          feed_dict={self.tar_img: batch_feat,
                          self.tar_txt: batch_vec,
                          self.pos_txt: batch_vec,
                          self.neg_txt: img_neg_txt,
                          ...
                          self.y_single: np.transpose([batch_labels]),
                          self.l: l})
                print('Epoch: [%2d][%4d/%4d] time: %4.4f, emb_loss: %.8f, domain_loss: %.8f, label_loss: %.8f, triplet_loss: %.8f, pair_loss: %.8f, g_loss: %.8f, domain_acc:%.8f, label_acc:%.8f, all_pair_acc:%.8f, pair_acc:%.8f, unpair_acc:%.8f' %(
                    epoch, idx, self.data_iter.num_train_batch, time.time() - start_time, emb_loss_val, domain_loss_val, label_loss_val, triplet_loss_val, pair_loss_val, g_loss_val, domain_acc_val, label_acc_val, pair_all_acc_val, pair_acc_val, unpair_acc_val
                ))
